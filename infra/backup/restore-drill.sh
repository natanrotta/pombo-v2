...
#
# Uso:
#   AGE_KEY_FILE=/caminho/seguro/backup-age.key /opt/boilerplate/restore-drill.sh
...
: "${TEST_CONTAINER:=boilerplate-db-test}"
: "${PG_USER:=boilerplate}"
: "${TIER:=daily}"
: "${TMP_DIR:=/tmp}"
...
docker run -d --rm --name "${TEST_CONTAINER}" \
  ...
  -e POSTGRES_DB=boilerplate_restore \
...
age -d -i "${AGE_KEY_FILE}" "${TMP_DIR}/${LATEST}" | \
  docker exec -i "${TEST_CONTAINER}" \
    pg_restore -U "${PG_USER}" -d boilerplate_restore --clean --if-exists --no-owner

rm -f "${TMP_DIR}/${LATEST}"

echo "[restore-drill] OK — restaurado em ${TEST_CONTAINER}:boilerplate_restore"
echo "[restore-drill] valide:   docker exec ${TEST_CONTAINER} psql -U ${PG_USER} -d boilerplate_restore -c '\\dt'"
